...
nll <- function(pars) {
    ...
    prYB.A1.B0[is.na(prYB.A1.B0)] <- 0
    prY.A1.B0 <- prYA.A1.B0 + prYB.A1.B0

    prYA.A0.B1 <- dbinom(yA, 1, fpA.B1, log=TRUE)
    prYB.A0.B1 <- dbinom(yB, 1, pB.A0, log=TRUE)
    prYA.A0.B1[is.na(prYA.A0.B1)] <- 0
