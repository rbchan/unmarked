...
SEXP nll_gdistsamp( SEXP y_, SEXP Xlam_, SEXP Xphi_, SEXP Xp_, SEXP beta_lam_, SEXP beta_phi_, SEXP beta_p_, SEXP log_alpha_, SEXP Xlam_offset_, SEXP Xphi_offset_, SEXP Xp_offset_, SEXP M_, SEXP mixture_, SEXP numPrimary_ ) {
  ...
  for(int i=0; i<R; i++) {
    ...
    ghi.zeros();
    for(int t=0; t<T; t++) {
      // if-else over different survey methods and det funs
      cp.zeros()
      cp(0) = // first multinomial cell prob
      for(int j=1; j<j; j++) {
	cp(j)
      }
      gh.zeros();
...
//	    }
	  h = lgamma(n+1) + sum(y(i,,t) * log(pi(i,,t)) -
				 lgamma(y(i,,t)+1))
	  }
	  gh(n,m) = g + h;
	}
