...
covariates, which is often of central interest, and because metapopulation 
processes such as local colonization and extinction can be directly estimated. 

Both the metapopulation design and absence of individual recognition 
distinguish this study design from another large class of ecological 
sampling methods known as `capture-recapture' methods. A vast number of 
models have been developed for capture-recapture data, and many of them 
...
class of hierarchical models \citep{royleDorazio08} has been developed that 
offers a unified 
...
occurrence at each site, but because imperfect detection prevents direct
observation of these quantities, these models regard actual abundance or 
occurrece as a latent variable. These models greatly improve upon 
traditional models that focused solely on modeling 
the observation process and used {\it ad hoc} methods of `correcting' 
observed data for detection probability. 
...
and detection level.  In addition, data are often accompanyed by metadata 
such as duration between surveys, making it difficult to utilize standard 
\rlang\ object classes to store the data and metadata. 
For these reasons, \um\ uses a new data 
type called the \code{unmarkedFrame} (Section~\ref{sec:data-requirements}).  
\um\ provides functions that import data from various common formats and
 convert them into \um's data types.  Once imported, \um\ provides functions to 
...
the function \code{occuRN} that estimates abundance from occurrence data. 
