...
class of hierarchical models \citetext{see \citep{royleDorazio08} for a 
...
occurrence at each site, but due to imperfect detection, these quantities are regarded actual abundance or 
occurrence as a latent variable. These models greatly improve upon 
...
the function \code{occuRN} that to fit the model of \citet{Royle2003} that
estimates abundance from detection, non-detection data. 
...
As the single-season occupancy model was generalized for multi-season data, 
the counterpart of this model was proposed by that relaxes the population closure 
assumption and may be fit using the \code{pcountOpen}.  
